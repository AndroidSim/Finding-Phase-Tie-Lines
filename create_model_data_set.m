function modeldataset = create_model_data_set(x,C_coex,S_coex,C_bdy,S_bdy,bdyconfig,tlconfig,Kpconfig,varargin)
    ...
    case 'parallel'
        switch bdyconfig
            case '2critpts'
                ...
                Ccoex = cart2tern(C_coex,1);
                Cbdy = cart2tern(C_bdy,1);
                B = repmat(output.spectra.magnetic_fields,1,ncS);
                Scoex = zeros(ncB,ncS*2);
                Scoex(:,1:2:end) = B;
                B = repmat(output.spectra.magnetic_fields,1,nbS);
                Sbdy = zeros(nbB,nbS*2);
                Sbdy(:,1:2:end) = B;
                if noise
                    Scoex(:,2:2:end) = output.spectra.theory_cS + sqrt(var_noise).*randn(ncB,ncS);
                    Sbdy(:,2:2:end) = S_bdy + sqrt(var_noise).*randn(nbB,nbS);
                else
                    Scoex(:,2:2:end) = output.spectra.theory_cS;
                    Sbdy(:,2:2:end) = S_bdy;
                end
                modeldataset = struct('Ccoex',Ccoex,'Scoex',Scoex,'Cbdy',Cbdy,'Sbdy',Sbdy,'output',output);
