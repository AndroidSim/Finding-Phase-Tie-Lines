function v = circle_bdy_fit_fxn(p,xdata)
...
elseif any(size(traj) == 1) & ~all(size(traj) == 1) % ie if vector
    ...
    error('2nd argument must be a matrix specifying trajectory end points');
end
...
elseif any(size(crit_pts) == 1) & ~all(size(crit_pts) == 1) % ie if vector
    ...
    error('1st argument must be scalar or matrix specifying tie lines');
end
...
mc1 = angle2slope(ang_cpt1);
mc2 = angle2slope(ang_cpt2);
